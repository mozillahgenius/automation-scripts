...
function onOpen() {
  ...
  ui.createMenu('YouTube管理')
    ...
    .addSeparator()
    .addSubMenu(ui.createMenu('ブランドアカウント診断')
      .addItem('チャンネル診断', 'diagnoseManagedChannels')
      .addItem('Analytics接続テスト', 'testAnalyticsWithBrandChannel'))
    .addSeparator()
...
function createConfigSheet() {
  ...
  var instructions = [
    ...
    ['・「ブランドアカウント診断」メニューでチャンネル状況を確認できます']
  ];
...
function diagnoseManagedChannels() {
  ...
  // 3. 設定シートのAnalyticsチャンネルID
  var settings = getGeneralSettings();
  if (settings.analyticsChannelId) {
    Logger.log('');
    Logger.log('【3】設定シートのAnalyticsチャンネルID:');
    Logger.log('  ID: ' + settings.analyticsChannelId);
    try {
      var brandResponse = YouTube.Channels.list('snippet,statistics', { id: settings.analyticsChannelId });
  ...
  Logger.log('');
  Logger.log('=== 診断終了 ===');
  Logger.log('実行ログで結果を確認してください。');

  showAlert('チャンネル診断が完了しました。\n\n' +
    'Apps Scriptエディタの「実行ログ」で\n' +
    '詳細な結果を確認してください。');
}
...
function testAnalyticsWithBrandChannel() {
  var settings = getGeneralSettings();
  var brandChannelId = settings.analyticsChannelId || getMyChannelId();

  if (!brandChannelId) {
    showAlert('チャンネルIDが設定されていません。\n\n' +
      '設定シートの「AnalyticsチャンネルID」に\n' +
      'チャンネルIDを入力してください。');
    return;
  }
  ...
  try {
    ...
    if (response.rows && response.rows.length > 0) {
      ...
      showAlert('Analytics接続テスト成功！\n\n' +
        'チャンネルID: ' + brandChannelId + '\n' +
        '取得件数: ' + response.rows.length + '件\n\n' +
        '詳細は実行ログで確認してください。');
    } else {
      Logger.log('データなし - 権限の問題の可能性があります');
      showAlert('Analytics接続テスト: データなし\n\n' +
        'チャンネルID: ' + brandChannelId + '\n\n' +
        '権限の問題またはデータがない可能性があります。');
    }

  } catch (e) {
    Logger.log('エラー: ' + e.message);

    var errorMsg = 'Analytics接続テストでエラーが発生しました。\n\n' +
      'エラー: ' + e.message + '\n\n';
    ...
    if (e.message.indexOf('forbidden') !== -1 || e.message.indexOf('403') !== -1) {
      ...
      Logger.log('3. 設定シートの「AnalyticsチャンネルID」が正しいか確認');

      errorMsg += '【解決方法】\n' +
        '1. YouTube Analytics APIが有効か確認\n' +
        '2. ブランドチャンネルのOwner権限を確認\n' +
        '3. チャンネルIDが正しいか確認\n' +
        '4. スクリプトを再認証してみてください';
    }
...
function showHelp() {
  var helpText = 'YouTube管理ツール ヘルプ（統合版）\n\n' +
    '【初期設定の流れ】\n' +
    '1. 「初期セットアップ」を実行\n' +
