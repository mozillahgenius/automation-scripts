...
// 安全なJSON抽出（コードブロックや余分なテキストを除去し、
// 途中で切れた場合でも最後に完全に閉じている位置までを切り出す）
function extractJsonBlock(text) {
  if (text == null) throw new Error('extractJsonBlock: input is null/undefined');
  text = String(text);
  // コードブロック除去
  ...
  // 文字列のバランスを取りながら、最後に完全に閉じた位置を探す
  let balCurly = 0, balSquare = 0, inStr = false, esc = false, lastSafe = -1;
  for (let i = start; i < text.length; i++) {
    ...
    if (inStr) {
      ...
      if (balCurly === 0 && balSquare === 0 && i > start) {
        lastSafe = i + 1;
        // 一旦完全に閉じた。以降に別のJSONが続くケースは基本無視
      }
    }
  }
  const end = lastSafe !== -1 ? lastSafe : text.length; // 可能なら完全に閉じたところまで
  return text.slice(start, end).trim();
}

// 安全なJSONパース（軽量な修復を試みる）
function safeParseJson(raw) {
  ...
  try {
    ...
    let cleaned = candidate
      ...
      // 制御文字の除去
      .replace(/[\u0000-\u001F]/g, '')
      // 未クローズの文字列を最低限修正（改行までの未閉じクォートを閉じる）
      ;
    
    // 途中で切れた文字列/要素を修復する試み
    // 1) 未終了の文字列リテラルを検出して閉じる
    if (cleaned.match(/"[^"]*$/)) {
      cleaned += '"';
    }
    // 2) 直前のカンマ以降の不完全要素を切り落とす
    (function pruneDanglingMember() {
      const lastOpenObj = Math.max(cleaned.lastIndexOf('{'), 0);
      const lastOpenArr = Math.max(cleaned.lastIndexOf('['), 0);
      const lastBoundary = Math.max(lastOpenObj, lastOpenArr);
      const lastComma = cleaned.lastIndexOf(',');
      if (lastComma > lastBoundary) {
    ...
    try {
      ...
      // さらに、最後の閉じカッコでトリミング（既にextractJsonBlockで実施済みだが念のため）
...
function getSlackChannelName(channelId) {
  ...
  try {
    ...
    // Botがメンバーでないチャンネルの場合は正常な動作なのでログ出力しない
    // チャンネルIDをそのまま返す
  }
  return channelId;
}
...
【重要な指示】
メッセージ内容に重要な議題や通知すべき事項がない場合は、必ず以下の通り明記してください：
- priority: "LOW"
- priorityReason: "特段の通知事項はありません"
- summary: "分析の結果、特段通知すべき事項はありません"
...
3. 重要度判定（HIGH/MEDIUM/LOW）とその理由
   - HIGH: 上記の「重要な議題の判定基準」に該当する場合のみ
   - MEDIUM: 500万円以上の予算、課長級の人事、部門間調整が必要
   - LOW: それ以外（日常業務・通常の連絡事項）
   ※重要な議題がない場合は必ずLOWを選択し、「特段の通知事項はありません」と明記
4. 具体的なアクションアイテム（重大な決定・承認が必要なもののみ、なければ空配列）
5. 関係者のリスト（意思決定に関わる重要人物のみ、なければ空配列）
6. 緊急度の評価（immediate/urgent/normal）
7. 決定事項（役員レベルで決定すべき事項のみ、なければ空配列）
8. リスク要因（事業に重大な影響があるリスクのみ、なければ空配列）
...
以下の構造のJSONを返してください（コードブロックなしで純粋なJSONのみ）:
{
  ...
  try {
    console.log('AI分析開始: gpt-5モデルを使用');
    ...
    try {
      ...
      console.error('Response (first 500 chars):', String(response).substring(0, 500));
      console.error('Response length:', String(response).length);
    ...
    result.priority = result.priority || 'LOW';  // デフォルトをLOWに変更
    ...
    // 重要な議題がない場合の処理
    if (result.topics.length === 0 && result.actionItems.length === 0 &&
        result.decisions.length === 0 && result.risks.length === 0) {
      result.priority = 'LOW';
      result.priorityReason = result.priorityReason || '特段の通知事項はありません';
      result.summary = result.summary || '分析の結果、特段通知すべき事項はありません';
    } else {
      result.summary = result.summary || 'メッセージを分析しました';
    }

  // 重要な論点のみをフィルタリング（新規追加）
  result.topics = filterCriticalTopics(result.topics, result.priority);
    ...
    // エラー時のデフォルト分析結果（通知事項なしとして処理）
    const defaultResult = {
      categories: ['その他'],
      topics: [],
      priority: 'LOW',
      priorityReason: '特段の通知事項はありません（分析エラーのためデフォルト値）',
      actionItems: [],
      stakeholders: [],
      urgency: 'normal',
      deadline: '',
      ...
      resources: { human: [], financial: '', time: '' },
      kpis: [],
      summary: `${messages.length}件のメッセージを処理しました。分析の結果、特段通知すべき事項はありません。`,
...
function performComprehensiveGovernanceCheck(messages, analysisResult) {
  const checkResult = {
    ...
    riskReasons: [], // リスク評価の理由
    complianceGaps: [],
    complianceEvaluation: {}, // コンプライアンスカテゴリ毎の評価
    tseDisclosureEvaluation: {}, // 東証開示条件の評価
    auditPoints: [],
  ...
  Object.entries(disclosureKeywords).forEach(([type, keywords]) => {
    if (keywords.some(keyword => combinedText.includes(keyword))) {
      ...
      checkResult.riskReasons.push(`開示要件（${type}）が必要なため、規制リスクが高い`);
    }
  });
  ...
  // リスクレベル評価（包括的な評価ロジック）
  const riskEvaluation = evaluateComprehensiveRiskLevel(messages, analysisResult, checkResult);
  checkResult.riskLevel = riskEvaluation.level;
  checkResult.riskReasons = [...checkResult.riskReasons, ...riskEvaluation.reasons];
  ...
  // AIを使用した包括的コンプライアンス評価
  const complianceEval = evaluateComplianceWithAI(messages, analysisResult);
  checkResult.complianceEvaluation = complianceEval;
  
  // 東証開示条件のAI判定
  const tseEval = evaluateTSEDisclosureRequirement(messages, analysisResult);
  checkResult.tseDisclosureEvaluation = tseEval;
  
  // 東証開示が必要な場合の処理
  if (tseEval.requiresDisclosure) {
    checkResult.requiresDisclosure = true;
    checkResult.disclosureType = tseEval.disclosureType || '適時開示';
    
    // 緊急度に応じたリスクレベルの調整
    if (tseEval.urgency === 'HIGH') {
      if (checkResult.riskLevel !== 'HIGH') {
        checkResult.riskLevel = 'HIGH';
        checkResult.riskReasons.push('東証開示要件に該当し、緊急度が高いため');
      }
    } else if (tseEval.urgency === 'MEDIUM' && checkResult.riskLevel === 'LOW') {
      checkResult.riskLevel = 'MEDIUM';
      checkResult.riskReasons.push('東証開示要件に該当する可能性があるため');
    }
  }
...
// ========= 包括的リスクレベル評価関数 =========
function evaluateComprehensiveRiskLevel(messages, analysisResult, governanceCheck) {
  const evaluation = {
    level: 'LOW',
    reasons: []
  };
  ...
  const financialKeywords = {
    high: ['損失', '赤字', '倒産', '債務超過', '資金ショート', '流動性危機'],
    medium: ['売上減少', '利益率低下', '予算超過', 'キャッシュフロー悪化', '与信問題'],
    low: ['予算内', '計画通り', '黒字', '利益確保']
  };
  
  const combinedText = messages.map(m => m.text || '').join(' ') + ' ' + JSON.stringify(analysisResult);
  
  // 財務リスクチェック
  if (financialKeywords.high.some(keyword => combinedText.includes(keyword))) {
    riskScore += 3;
    evaluation.reasons.push('財務上の重大な懸念事項（損失・赤字・資金問題等）が検出された');
  } else if (financialKeywords.medium.some(keyword => combinedText.includes(keyword))) {
    riskScore += 2;
    evaluation.reasons.push('財務パフォーマンスに関する懸念（売上減少・利益率低下等）が存在');
  }
  
  // 2. 法的・コンプライアンスリスク
  const legalKeywords = {
    high: ['訴訟', '違法', '法令違反', '処分', '罰則', '刑事責任', '行政指導'],
    medium: ['契約違反', 'クレーム', '紛争', '係争', '法的リスク', 'コンプライアンス違反'],
    low: ['法令遵守', '適法', 'コンプライアンス確保']
  };
  
  if (legalKeywords.high.some(keyword => combinedText.includes(keyword))) {
    riskScore += 3;
    evaluation.reasons.push('法的リスク（訴訟・法令違反・処分等）が認められる');
  } else if (legalKeywords.medium.some(keyword => combinedText.includes(keyword))) {
    riskScore += 2;
    evaluation.reasons.push('潜在的な法的リスク（契約違反・紛争等）が存在');
  }
  
  // 3. オペレーショナルリスク
  const operationalKeywords = {
    high: ['システム停止', '重大事故', 'データ漏洩', 'セキュリティ侵害', '個人情報流出'],
    medium: ['障害', 'インシデント', '遅延', '品質問題', 'ミス', 'エラー'],
    low: ['正常稼働', '問題なし', '順調']
  };
  
  if (operationalKeywords.high.some(keyword => combinedText.includes(keyword))) {
    riskScore += 3;
    evaluation.reasons.push('重大なオペレーショナルリスク（システム停止・データ漏洩等）が発生');
  } else if (operationalKeywords.medium.some(keyword => combinedText.includes(keyword))) {
    riskScore += 2;
    evaluation.reasons.push('業務運営上のリスク（障害・品質問題等）が存在');
  }
  
  // 4. 戦略リスク
  const strategicKeywords = {
    high: ['事業撤退', '大幅な戦略転換', '競合に敗北', '市場シェア急落'],
    medium: ['競争激化', '市場環境悪化', '新規参入者', '代替品脅威'],
    low: ['市場リード', '競争優位', '成長軌道']
  };
  
  if (strategicKeywords.high.some(keyword => combinedText.includes(keyword))) {
    riskScore += 3;
    evaluation.reasons.push('戦略的な重大リスク（事業撤退・市場喪失等）が存在');
  } else if (strategicKeywords.medium.some(keyword => combinedText.includes(keyword))) {
    riskScore += 2;
    evaluation.reasons.push('競争環境の変化による戦略リスクが認められる');
  }
  
  // 5. レピュテーションリスク
  const reputationKeywords = {
    high: ['炎上', 'スキャンダル', '不祥事', '信用失墜', 'ブランド毀損'],
    medium: ['批判', 'ネガティブな評価', '苦情増加', '評判低下'],
    low: ['高評価', '信頼獲得', 'ブランド向上']
  };
  
  if (reputationKeywords.high.some(keyword => combinedText.includes(keyword))) {
    riskScore += 3;
    evaluation.reasons.push('深刻なレピュテーションリスク（炎上・不祥事等）が発生');
  } else if (reputationKeywords.medium.some(keyword => combinedText.includes(keyword))) {
    riskScore += 2;
    evaluation.reasons.push('評判・ブランドに関する懸念が存在');
  }
  
  // 6. ガバナンス要因によるリスク評価
  if (governanceCheck.requiresApproval && governanceCheck.approvalLevel === '株主総会') {
    riskScore += 2;
    evaluation.reasons.push('株主総会決議が必要な重要事項である');
  } else if (governanceCheck.requiresApproval && governanceCheck.approvalLevel === '取締役会') {
    riskScore += 1;
    evaluation.reasons.push('取締役会決議が必要な案件である');
  }
  
  // 7. 分析結果からのリスク評価
  if (analysisResult.risks && analysisResult.risks.length > 0) {
    const highRisks = analysisResult.risks.filter(r => r.impact === 'high').length;
    const mediumRisks = analysisResult.risks.filter(r => r.impact === 'medium').length;
    
    if (highRisks > 0) {
      riskScore += highRisks * 2;
      evaluation.reasons.push(`分析により${highRisks}件の高インパクトリスクが識別された`);
    }
    if (mediumRisks > 0) {
      riskScore += mediumRisks;
      evaluation.reasons.push(`分析により${mediumRisks}件の中程度のリスクが識別された`);
    }
  }
  
  // 8. 緊急性による加点
  const urgencyKeywords = ['緊急', '至急', '即座に', '今すぐ', '直ちに'];
  if (urgencyKeywords.some(keyword => combinedText.includes(keyword))) {
    riskScore += 1;
    evaluation.reasons.push('緊急対応が必要な事項が含まれている');
  }
  ...
  if (riskScore >= 8) {
    ...
    if (evaluation.reasons.length === 0) {
      evaluation.reasons.push('複合的な要因により総合的にリスクが高いと判断');
    }
  } else if (riskScore >= 4) {
    evaluation.level = 'MEDIUM';
    if (evaluation.reasons.length === 0) {
      evaluation.reasons.push('一定のリスク要因が認められる');
    }
  } else {
    evaluation.level = 'LOW';
    if (evaluation.reasons.length === 0) {
      evaluation.reasons.push('現時点で重大なリスク要因は認められない');
    }
  }
...
function slackAPI(method, params = {}) {
  ...
  try {
    // conversations.infoの呼び出しはログを出力しない（頻繁に呼ばれ、エラーも想定内のため）
    ...
    if (!data.ok) {
      // 想定内のエラー（Botがメンバーでないチャンネルへのアクセス等）はログを出力しない
      const expectedErrors = ['invalid_arguments', 'not_in_channel', 'channel_not_found'];
      const isExpectedError = expectedErrors.includes(data.error) && method === 'conversations.info';
...
function syncSlackMessages() {
  ...
  if (channelIds.length === 0) {
    ...
    console.error('エラー: 監視対象チャンネルが設定されていません。Configシートを確認してください。');
    sendEmailNotification(
      'Slack同期エラー',
      '監視対象チャンネルが設定されていません。Configシートを確認してください。'
    );
    return;
  }
  ...
  console.log(`同期完了: Slackメッセージの同期が完了しました。\n${resultMessage}\n合計: ${totalMessageCount}件のメッセージを保存`);
...
function analyzeHistoricalMessages() {
  ...
  if (!slackLogSheet) {
    console.error('slack_logシートが見つかりません');
    logError('過去ログ分析', 'slack_logシートが見つかりません');
    return { success: false, error: 'slack_logシートが見つかりません' };
  }
  ...
  console.log(`分析完了: ${messages.length}件のメッセージを処理しました`);
  logInfo(`過去ログ分析完了: ${messages.length}件を処理`);
...
function generateManualAndFAQFromMessages() {
  ...
  if (!slackLogSheet) {
    console.error('slack_logシートが見つかりません');
    logError('マニュアル・FAQ生成', 'slack_logシートが見つかりません');
    return { success: false, error: 'slack_logシートが見つかりません' };
  }
  ...
  if (messages.length === 0) {
    console.log('過去24時間にメッセージがありません');
    return { success: true, manuals: 0, faqs: 0, message: '過去24時間にメッセージがありません' };
  }
  ...
  const resultMessage = `生成完了 - マニュアル: ${totalManuals}件, FAQ: ${totalFAQs}件`;
  console.log(resultMessage);
  logInfo(`マニュアル・FAQ生成完了: マニュアル${totalManuals}件, FAQ${totalFAQs}件`);
...
function callOpenAIForAgenda(messages, model = 'gpt-5', responseFormat = null) {
  ...
  const url = OPENAI_URL_RESPONSES;
  ...
  try {
    ...
    const content = extractTextFromOpenAIResponse_(data);
    if (responseFormat && responseFormat.type === 'json_object') {
...
function runAIAnalysis() {
  ...
  if (!messagesSheet) {
    console.error('エラー: Messagesシートが見つかりません。先にメッセージを同期してください。');
    logError('AI分析', 'Messagesシートが見つかりません');
    return { success: false, error: 'Messagesシートが見つかりません' };
  }
  
  // 未分析のメッセージを取得
  const lastRow = messagesSheet.getLastRow();
  if (lastRow <= 1) {
    console.log('情報: 分析対象のメッセージがありません。');
    return { success: true, analyzed: 0, message: '分析対象のメッセージがありません' };
  }
  ...
  if (unanalyzedMessages.length === 0) {
    console.log('情報: 全てのメッセージは既に分析済みです。');
    return { success: true, analyzed: 0, message: '全てのメッセージは既に分析済みです' };
  }
  
  console.log(`分析開始: ${unanalyzedMessages.length}件のメッセージを分析します。`);
  ...
  const resultMessage = `分析完了！ 成功: ${analyzedCount}件, エラー: ${errorCount}件`;
...
function analyzeSlackAndSendReport() {
  ...
  if (!agendaItems || agendaItems.length === 0) {
    logInfo('重要な議題が見つかりませんでした');
    
    // 議題がない場合のレポート生成（スプレッドシートなし）
    const report = {
      date: now,
      messageCount: recentMessages.length,
      agendaItems: [],
      flowchart: null,
      summary: '分析の結果、現時点で重要な議題は見つかりませんでした。',
      spreadsheetUrl: null // スプレッドシートURLを明示的にnullに
    };
  ...
  // 6. 新規スプレッドシートにレポートをエクスポート（議題がある場合のみ）
...
// SlackメッセージURL（簡易生成版）
// chat.getPermalink の代替として、チームドメイン不要の共通URLを生成する
// スレッドの場合は親メッセージのURLに thread_ts と cid をクエリで付与
function buildSlackPermalink(channelId, messageTs, threadTs) {
  if (!channelId || !messageTs) return '';
  const tsForUrl = String(messageTs).replace('.', '');
  if (threadTs && threadTs !== messageTs) {
    const threadTsForUrl = String(threadTs).replace('.', '');
    return `https://slack.com/archives/${channelId}/p${threadTsForUrl}?thread_ts=${threadTs}&cid=${channelId}`;
  }
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][8] === true && !data[i][9]) { // match_flag=true かつ human_judgement未設定
      try {
        ...
        // permalinkがない場合は、channel_idとmessage_tsから生成を試みる
        let permalink = data[i][10];
        if (!permalink && data[i][1] && data[i][2]) {
          // channel_id (data[i][1]) と message_ts (data[i][2]) から生成
          const channelId = String(data[i][1]).trim();
          const messageTs = String(data[i][2]).trim();
          const threadTs = data[i][3] ? String(data[i][3]).trim() : '';
          
          if (channelId && messageTs && channelId !== 'undefined' && messageTs !== 'undefined') {
            const tsForUrl = messageTs.replace('.', '');
            if (threadTs && threadTs !== messageTs && threadTs !== 'undefined') {
              // スレッドの場合
              const threadTsForUrl = threadTs.replace('.', '');
              permalink = `https://slack.com/archives/${channelId}/p${threadTsForUrl}?thread_ts=${threadTs}&cid=${channelId}`;
            } else {
              // 通常メッセージの場合
              permalink = `https://slack.com/archives/${channelId}/p${tsForUrl}`;
            }
            
            // 生成したpermalinkをシートに保存
            sheet.getRange(i + 1, 11).setValue(permalink);
            console.log(`permalink生成・保存: 行${i+1}, URL: ${permalink}`);
          } else {
            console.log(`permalink生成スキップ: 行${i+1}, channelId=${channelId}, messageTs=${messageTs}`);
          }
        }
...
function sendSlackNotification(channel, matches) {
  // チャンネルIDのクリーニング（#や<>を除去）
  const cleanChannelId = channel.replace(/^[#<]|[>]$/g, '').trim();
  
  console.log(`Slack通知送信: チャンネル ${cleanChannelId} に ${matches.length} 件の議題を送信`);
  ...
  matches.forEach((match, index) => {
    ...
    const messageText = `*${index + 1}. ${match.summary.summary}*\n` +
  ...
  try {
    const response = slackAPI('chat.postMessage', {
      ...
      blocks: blocks,
      text: `議題候補: ${matches.length}件` // フォールバックテキスト
    });
    
    if (response && response.ok) {
      console.log(`Slack通知送信成功: ${cleanChannelId}`);
      logInfo(`Slack通知送信成功: ${cleanChannelId}`);
    }
  } catch (error) {
    console.error(`Slack通知エラー: ${error}`);
    logError('Slack通知', error.toString());
    
    // エラーの詳細を記録
    if (error.toString().includes('channel_not_found')) {
      console.error(`チャンネル ${cleanChannelId} が見つかりません。以下を確認してください:`);
      console.error('1. チャンネルIDが正しいか（#を含まない）');
      console.error('2. Botがチャンネルに招待されているか');
      console.error('3. チャンネルが存在するか');
      
      // 診断関数を実行
      try {
        diagnoseNotificationChannel(cleanChannelId);
      } catch (diagError) {
        console.error('診断中にエラー:', diagError);
      }
    }
...
<body>
  ...
  ${matches.map((match, index) => {
    // デバッグ用ログ
    console.log(`議題候補 ${index + 1}: permalink = ${match.permalink}`);
    ...
    <div class="match">
      ...
      ${match.permalink ? `
      <div class="link">
        <a href="${match.permalink}" style="color: #1264a3; text-decoration: none; font-weight: 500;">
          🔗 Slackスレッドを開く
        </a>
      </div>
      ` : `
      <div class="link" style="color: #999; font-style: italic;">
        （Slackリンクが利用できません）
      </div>
      `}
    </div>
...
`;
  ...
  // プレーンテキスト版も作成
  let plainBody = `【議題候補】${matches.length}件の該当案件\n\n`;
  
  matches.forEach((match, index) => {
    plainBody += `${index + 1}. ${match.summary.summary}\n`;
    plainBody += `   カテゴリ: ${match.topCategory}\n`;
    plainBody += `   スコア: ${match.topScore}\n`;
    if (match.permalink) {
      plainBody += `   Slackスレッド: ${match.permalink}\n`;
    } else {
      plainBody += `   （Slackリンクが利用できません）\n`;
    }
    plainBody += '\n';
  });
  
  plainBody += `\n詳細はスプレッドシートをご確認ください:\n`;
  plainBody += `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}\n`;
...
function getRecentSlackMessages(slackLogSheet, startDate, endDate) {
  ...
  const threadTsIndex = headers.indexOf('thread_ts');
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (messageDate >= startDate && messageDate <= endDate) {
      ...
      const ts = data[i][tsIndex] || '';
      const threadTs = data[i][threadTsIndex] || '';
    ...
    try {
      ...
      if (analysis && analysis.agendas) {
        analysis.agendas.forEach(agenda => {
          // 関連するメッセージのpermalinkを収集
          const relatedPermalinks = channelMessages
            .filter(msg => msg.permalink)
            .slice(0, 5) // 最大5件のリンクを保持
            .map(msg => msg.permalink);
          ...
          agendaItems.push({
            ...
            permalinks: relatedPermalinks // Slack URLを追加
          });
        });
      }
    } catch (e) {
...
`;
  ...
  try {
    ...
    const apiPayload = {
      model: 'gpt-5',
      input: `SYSTEM:\n日本語のSlackメッセージから重要な議題を抽出する専門家として動作してください。\n\nUSER:\n${prompt}`,
      max_output_tokens: 1000,
      text: { format: { type: 'json_schema', name: 'agenda', schema: { type: 'object', properties: {}, additionalProperties: true }, strict: false } }
    };
    
    // 追加のモデル特化設定が必要な場合はここで設定
    ...
    const result = JSON.parse(response.getContentText());
    const content = extractTextFromOpenAIResponse_(result);
...
function createAgendaReportHtml(report) {
  ...
  // spreadsheetUrlがnullまたは未設定の場合はスプレッドシートリンクを表示しない
  const spreadsheetUrl = report.spreadsheetUrl;
...
<body>
  ...
  <div class="summary">
    ...
    ${spreadsheetUrl && spreadsheetUrl !== null && spreadsheetUrl !== undefined ? `
    <div style="text-align: center; margin-top: 20px;">
      <a href="${spreadsheetUrl}" class="spreadsheet-button" target="_blank">
        📊 スプレッドシートを見る
      </a>
    </div>
    ` : ''}
...
`;
  ...
  if (report.agendaItems && report.agendaItems.length > 0) {
    ...
    report.agendaItems.forEach((item, index) => {
      ...
      <div class="agenda-item priority-${item.priority}">
        ...
        ${item.permalinks && item.permalinks.length > 0 ? `
        <p><strong>📎 Slackスレッド:</strong></p>
        <ul style="margin-top: 5px;">
          ${item.permalinks.map((link, idx) => `
            <li><a href="${link}" style="color: #1264a3; text-decoration: none;">
              🔗 関連メッセージ ${idx + 1} を見る
            </a></li>
          `).join('')}
        </ul>
  ...
  <div class="footer">
    ...
    <p style="font-size: 12px; margin-top: 10px;">
      <em>詳細な分析結果と業務フローチャート、専門家への相談事項は上記のスプレッドシートからご確認ください。</em>
    </p>
    ` : ''}
...
function createAgendaReportPlainText(report) {
  ...
  if (spreadsheetUrl && spreadsheetUrl !== null && spreadsheetUrl !== undefined) {
    text += '【スプレッドシート】\n';
    text += `詳細な分析結果と業務フローチャート、専門家への相談事項は以下からご確認ください：\n`;
    text += `${spreadsheetUrl}\n\n`;
  }
  ...
  if (report.agendaItems && report.agendaItems.length > 0) {
    ...
    report.agendaItems.forEach((item, index) => {
      ...
      if (item.permalinks && item.permalinks.length > 0) {
        text += `   Slackスレッド:\n`;
        item.permalinks.forEach((link, idx) => {
          text += `     - ${link}\n`;
        });
      }
    });
...
function exportReportToNewSpreadsheet(report) {
  ...
  // 高優先度の議題だけに絞る
  const highPriorityItems = (report.agendaItems || []).filter(item => item.priority === 'high');
  ...
  const summaryData = [
    ...
    ['抽出された高優先度議題数', highPriorityItems.length],
    ['', ''],
    ['優先度別内訳', ''],
    ['高優先度', highPriorityItems.length],
  ...
  // 2. 議題詳細（高優先度のみ）シートを作成
  if (highPriorityItems.length > 0) {
    const agendaSheet = newSpreadsheet.insertSheet('議題詳細(高優先度)');
    ...
    // 議題データを書き込み
    const agendaData = highPriorityItems.map(item => [
      item.priority || '',
      item.title || '',
      item.description || '',
      item.actionRequired || '',
  ...
  // 3. 業務フローシートを作成（高優先度の議題に基づく）
  if (highPriorityItems.length > 0) {
    const highFlowchart = generateBusinessFlowchart(highPriorityItems);
    ...
    flowSheet.getRange(3, 1).setValue(highFlowchart).setWrap(true);
    flowSheet.setColumnWidth(1, 800);
  }
  
  // 4. ビジュアルフローシートを作成（高優先度の議題のみ）
  try {
    const visualFlowSheet = newSpreadsheet.insertSheet('ビジュアルフロー');
    generateVisualFlowFromAgenda(visualFlowSheet, highPriorityItems);
  ...
  // 5. 重要タスク詳細を作成（高優先度議題を深掘り）
  try {
    const tasksSheet = newSpreadsheet.insertSheet('重要タスク詳細');
    const taskHeaders = ['タスク', '背景/課題', '目的', '期待効果', '期限', '優先度', '担当候補', '関係者', '主要ステップ', '依存関係', 'リスク', '成功指標', '次のアクション', 'チャンネル', '参照リンク'];
    tasksSheet.getRange(1, 1, 1, taskHeaders.length).setValues([taskHeaders]).setFontWeight('bold');

    const taskDetails = generateTaskDetailsForHighPriority_(highPriorityItems);
    if (taskDetails.length > 0) {
      const rows = taskDetails.map((t, idx) => [
        t.title || highPriorityItems[idx]?.title || '',
        t.problem_statement || '',
        t.goal || '',
        t.impact || '',
        t.due_by || '',
        'HIGH',
        Array.isArray(t.owner_candidates) ? t.owner_candidates.join('\n') : (t.owner || ''),
        Array.isArray(t.stakeholders) ? t.stakeholders.join('\n') : '',
        Array.isArray(t.steps) ? t.steps.map(s => (typeof s === 'string' ? s : (s.step ? `- ${s.step}${s.details ? ': ' + s.details : ''}` : JSON.stringify(s)))).join('\n') : '',
        Array.isArray(t.dependencies) ? t.dependencies.join('\n') : '',
        Array.isArray(t.risks_mitigations) ? t.risks_mitigations.join('\n') : (Array.isArray(t.risks) ? t.risks.join('\n') : ''),
        Array.isArray(t.metrics) ? t.metrics.join('\n') : (Array.isArray(t.kpi_sla) ? t.kpi_sla.join('\n') : ''),
        Array.isArray(t.next_actions) ? t.next_actions.join('\n') : '',
        highPriorityItems[idx]?.channel || '',
        (highPriorityItems[idx]?.permalinks || []).join('\n')
      ]);
      tasksSheet.getRange(2, 1, rows.length, taskHeaders.length).setValues(rows);
      // テキスト列は折り返し
      tasksSheet.getRange(2, 2, rows.length, taskHeaders.length - 1).setWrap(true);
      // 列幅調整
      const widths = [240, 260, 220, 200, 100, 80, 180, 180, 280, 180, 220, 180, 220, 120, 260];
      for (let i = 0; i < widths.length; i++) tasksSheet.setColumnWidth(i + 1, widths[i]);
    }
  } catch (e) {
    logError('重要タスク詳細作成エラー', e.toString());
  }
...
// リトライ処理
function retryWithBackoff(func, maxRetries = null, baseDelayMs = null) {
  const configRetries = parseInt(getConfig('MAX_RETRY_COUNT') || '3');
  const retryDelay = parseInt(getConfig('RETRY_DELAY_MS') || '2000');
  let effectiveRetries = (typeof maxRetries === 'number' && !isNaN(maxRetries)) ? maxRetries : configRetries;
  // 最低10回は試行
  if (effectiveRetries < 10) effectiveRetries = 10;
  const effectiveDelay = (typeof baseDelayMs === 'number' && !isNaN(baseDelayMs)) ? baseDelayMs : retryDelay;
  
  for (let i = 0; i < effectiveRetries; i++) {
    try {
      return func();
    } catch (e) {
      if (i === effectiveRetries - 1) throw e;
      Utilities.sleep(effectiveDelay * Math.pow(2, i));
    }
  }
}
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; padding: 20px;">
      ...
      <label style="background-color: #e8f5e9; padding: 5px; border-radius: 5px; display:block; margin-bottom:8px;">
        ...
        <strong>GPT-5</strong> 🌟 (推奨) - 高精度な推論、Responses API最適化、構造化出力に強い
      </label>
...
function updateOpenAIModel(model) {
  ...
  if (model === 'gpt-5') {
    message += '\n\n🌟 GPT-5を使用します。Responses API最適化で、構造化出力と深掘りが高精度です。';
...
function upgradeToGPT4() {
  ...
  ui.alert('アップグレード完了', 'GPT-5モデルに切り替えました。Responses APIに最適化された高精度な出力が可能です。', ui.ButtonSet.OK);
...
// Responses API汎用テキスト抽出（後方互換対応）
function extractTextFromOpenAIResponse_(data) {
  try {
    if (!data) {
      console.warn('extractTextFromOpenAIResponse_: dataがnullまたはundefined');
      return '';
    }

    // gpt-5/o3系のレスポンス形式（v1/responses）
    if (typeof data.output_text === 'string' && data.output_text.trim()) {
      console.log('output_text形式で取得:', data.output_text.substring(0, 100));
      return data.output_text;
    }

    // 配列形式のoutput（v1/responses）
    if (Array.isArray(data.output)) {
      ...
      if (joined) {
        console.log('output配列形式で取得:', joined.substring(0, 100));
        return joined;
      }
    }

    // 通常のChat Completions形式（v1/chat/completions）
    if (data.choices && Array.isArray(data.choices) && data.choices.length > 0) {
      const ch = data.choices[0];

      // message.content形式
      if (ch.message && ch.message.content !== null && ch.message.content !== undefined) {
        const content = ch.message.content;
        if (typeof content === 'string') {
          console.log('choices[0].message.content形式で取得:', content.substring(0, 100));
          return content;
        } else if (typeof content === 'object') {
          // JSONオブジェクトの場合は文字列化
          const jsonStr = JSON.stringify(content);
          console.log('choices[0].message.content（JSON）形式で取得:', jsonStr.substring(0, 100));
          return jsonStr;
        }
      }
      ...
      if (typeof ch.text === 'string') {
        console.log('choices[0].text形式で取得:', ch.text.substring(0, 100));
        return ch.text;
      }
    }
    ...
    if (typeof data === 'string') {
      console.log('data自体が文字列:', data.substring(0, 100));
      return data;
    }

    // その他の形式の場合、JSON文字列化を試す
    if (typeof data === 'object') {
      const jsonStr = JSON.stringify(data);
      console.warn('未知の形式のため、JSON文字列化:', jsonStr.substring(0, 200));
      // エラー情報が含まれている場合
      if (data.error) {
        console.error('OpenAI APIエラー:', data.error);
        return '';
      }
    }

  } catch (e) {
    console.error('extractTextFromOpenAIResponse_ error:', e);
    console.error('処理対象data:', JSON.stringify(data).substring(0, 500));
  }

  console.warn('extractTextFromOpenAIResponse_: コンテンツを抽出できませんでした');
  return '';
}

// 高優先度議題を詳細タスクに深掘り
function generateTaskDetailsForHighPriority_(highPriorityItems) {
  const results = [];
  for (let i = 0; i < highPriorityItems.length; i++) {
    const item = highPriorityItems[i];
    try {
      const detail = generateTaskDetailForAgendaItem_(item);
      if (detail) results.push(detail);
      Utilities.sleep(400);
    } catch (e) {
      console.error('タスク詳細生成エラー:', e.toString());
    }
  }
  return results;
}
...
// タスク詳細スキーマ
function buildTaskDetailSchema_() {
  return {
    type: 'object',
    ...
    properties: {
      ...
      steps: { type: 'array', items: { anyOf: [ { type: 'string' }, { type: 'object', properties: { step: { type: 'string' }, details: { type: 'string' } }, additionalProperties: true } ] } },
...
// 単一の議題を詳細タスクに変換（gpt-5 Responses API）
function generateTaskDetailForAgendaItem_(item) {
  const apiKey = PropertiesService.getScriptProperties().getProperty('OPENAI_API_KEY') || OPENAI_API_KEY;
  if (!apiKey) throw new Error('OpenAI APIキーが設定されていません');

  const sys = 'あなたは一流のプロジェクトマネージャーです。与えられた議題から、実行可能で具体的なタスク仕様を日本語で出力してください。曖昧さを排除し、担当候補・期限・主要ステップ・依存関係・リスク・成功指標・次のアクションを必ず含めます。';
  const user = `入力議題:
- タイトル: ${item.title || ''}
- 説明: ${item.description || ''}
- 優先度: ${item.priority || ''}
- チャンネル: ${item.channel || ''}
- 関係者: ${(item.participants || []).join(', ')}
- キーワード: ${(item.keywords || []).join(', ')}
- 参照リンク:\n${(item.permalinks || []).map(l => '- ' + l).join('\n')}

要件:
1) タスクの背景/課題（problem_statement）
2) 目的（goal）と期待効果（impact）
3) 期限（due_by）
4) 担当候補（owner_candidates）と関係者（stakeholders）
5) 主要ステップ（steps）
6) 依存関係（dependencies）
7) リスク（risks / risks_mitigations）
8) 成功指標（metrics）
9) 次のアクション（next_actions）
日本語で、JSONのみを返してください。`;

  const schema = buildTaskDetailSchema_();

  const payload = {
    model: 'gpt-5',
    input: `SYSTEM:\n${sys}\n\nUSER:\n${user}`,
    max_output_tokens: 2000,
    text: { format: { type: 'json_schema', name: 'task_detail', schema: schema, strict: false } }
  };
  ...
  if (status !== 200) {
    throw new Error(`OpenAI error ${status}: ${res.getContentText().substring(0, 500)}`);
  }
  const data = JSON.parse(res.getContentText());
  const text = extractTextFromOpenAIResponse_(data);
  if (!text) return null;
  try {
    return JSON.parse(text);
  } catch (_) {
    return { title: item.title || '', problem_statement: text };
  }
}
...
function callOpenAI(mailBody, orgProfileJson) {
  ...
  const response = retryWithBackoff(() => {
    ...
    return res;
  }, isO3ModelForChat ? 10 : null, isO3ModelForChat ? 5000 : null);
...
function callOpenAIResponses(mailBody, orgProfileJson, apiKey, modelName, schema) {
  ...
  // schemaの最低限の妥当性を担保
  let ensuredSchema = schema || { type: 'object' };
  if (ensuredSchema && ensuredSchema.type === 'object' && ensuredSchema.properties == null) {
    ensuredSchema = Object.assign({ properties: {}, additionalProperties: true }, ensuredSchema);
  }
  ...
  const response = retryWithBackoff(() => {
    ...
    throw new Error(`OpenAI API error ${status}: ${errorBody}`);
  }, isO3Model ? 10 : null, isO3Model ? 5000 : null);
  ...
  // Responses APIの多様な戻り形式を解析
  const extract = (obj) => {
    if (!obj) return '';
    const outputs = Array.isArray(obj.output) ? obj.output : [];
    for (const out of outputs) {
      ...
      for (const part of contents) {
        if (part && typeof part === 'object') {
          ...
          if (part.type === 'output_structured') {
            return part.parsed || part.structured || part.json;
          }
          if (part.type === 'output_text' && typeof part.text === 'string') {
            try { return JSON.parse(part.text); } catch (_) { /* fallthrough */ }
          }
        }
      }
    }
    if (typeof obj.output_text === 'string') {
      try { return JSON.parse(obj.output_text); } catch (_) { /* fallthrough */ }
    }
    return '';
  };

  let parsed = extract(responseData) || (responseData.response ? extract(responseData.response) : '');
  if (parsed && typeof parsed === 'object') return parsed;
  if (typeof parsed === 'string' && parsed.trim() !== '') {
    try { return JSON.parse(parsed); } catch (_) { /* ignore */ }
  }
  // 最後の手段: 全文をログしてエラー
  console.warn('Responses APIから期待形式を抽出できませんでした。rawの先頭500文字:', response.getContentText().substring(0, 500));
  throw new Error('Failed to extract structured JSON from Responses API');
}
...
// 旧o3-deep-research切り替え関数は廃止（gpt-5へ統一）
...
function showCurrentModel() {
  ...
  // モデルの特徴を説明
  const modelFeatures = {
    'gpt-5': '次世代モデル - 高精度な推論とResponses API最適化、構造化出力に強い',
  ...
  try {
    ...
    const options = {
      ...
      payload: JSON.stringify({
        model: 'gpt-5',
        input: `SYSTEM:\nあなたは議事録作成と議題抽出の専門家です。重要な議題を見逃さないようにしてください。\n\nUSER:\n${prompt}`,
        max_output_tokens: 2000,
        text: { format: { type: 'json_schema', name: 'agenda', schema: { type: 'object', properties: {}, additionalProperties: true }, strict: false } }
      }),
      muteHttpExceptions: true
...
/**
 * 時間トリガーから実行する専用関数
 * UIコンテキストを使用しないように autoMode = true で実行
 */
function executeScheduledWorkflow() {
  console.log('===== 時間トリガーによるワークフロー実行開始 =====');
  console.log('実行時刻:', new Date().toLocaleString('ja-JP'));
  
  try {
    // autoMode = true でUIを使用しないように実行
    const result = executeIntegratedContinuousWorkflow(null, true);
    
    // 実行結果をログに記録
    console.log('ワークフロー実行結果:');
    console.log('- 成功:', result.success);
    console.log('- メッセージ数:', result.messages?.length || 0);
    console.log('- 分析優先度:', result.analysis?.priority || 'N/A');
    console.log('- ガバナンスリスク:', result.governance?.riskLevel || 'N/A');
    ...
    if (!result.success && result.errors.length > 0) {
      sendErrorNotification(new Error(result.errors[0]), result);
    }
    
    return result;
  } catch (error) {
    console.error('時間トリガー実行エラー:', error);
...
/**
 * 統合連続ワークフロー実行（UIなし版）
 * @param {string} channelId - 処理対象のチャンネルID（nullの場合は全チャンネル）
 * @param {boolean} autoMode - 自動モード（廃止予定、互換性のため残す）
 */
function executeIntegratedContinuousWorkflow(channelId = null, autoMode = true) {
  const startTime = new Date();
  console.log('===== 統合連続ワークフロー開始 =====');
  console.log(`実行時刻: ${startTime.toLocaleString('ja-JP')}`);

  // チャンネルIDの検証とログ出力の改善
  let targetDescription = '全チャンネル';
  if (channelId) {
    // チャンネルIDがオブジェクトとして誤って渡された場合の対処
    if (typeof channelId === 'object') {
      console.warn('警告: チャンネルIDがオブジェクトとして渡されました:', JSON.stringify(channelId));
      channelId = null; // 全チャンネルとして処理
    } else {
      targetDescription = String(channelId);
    }
  }
  console.log(`対象チャンネル: ${targetDescription}`);
  ...
  try {
    ...
    workflowLog.push(createLogEntry('START', 'ワークフロー開始', `チャンネル: ${targetDescription}`));
    ...
    if (!messages || messages.length === 0) {
      ...
      // UIを使用せず、ログとメール通知のみ
      console.warn('メッセージ取得失敗 - チャンネルアクセス権限を確認してください');
      console.log('【必要な設定】');
      console.log('1. Slackアプリをチャンネルに追加');
      console.log('2. 必要なスコープ:');
      console.log('   - channels:history');
      console.log('   - channels:read');
      console.log('   - groups:history (プライベートチャンネル用)');
...
`;
        ...
        try {
          MailApp.sendEmail({
            to: REPORT_EMAIL,
            subject: '[警告] Slackメッセージ取得失敗',
            body: errorMessage
          });
        } catch (mailError) {
          console.error('警告メール送信失敗:', mailError);
        }
      }
  ...
  console.log(`メッセージ数: ${workflowResult.messages?.length || 0}`);
  console.log(`分析優先度: ${workflowResult.analysis?.priority || 'N/A'}`);
  console.log(`ガバナンスリスク: ${workflowResult.governance?.riskLevel || 'N/A'}`);
  
  // 成功時のサマリーメール送信（オプション）
  if (workflowResult.success && REPORT_EMAIL) {
...
- 分析優先度: ${workflowResult.analysis?.priority || 'N/A'}
- ガバナンスリスク: ${workflowResult.governance?.riskLevel || 'N/A'}
- スプレッドシート: ${workflowResult.spreadsheetUrl || 'なし'}

実行時刻: ${new Date().toLocaleString('ja-JP')}
`;
    
    try {
      // 重要度が高い場合のみサマリーメールを送信
      if (workflowResult.analysis?.priority === 'HIGH' || workflowResult.governance?.requiresAction) {
        MailApp.sendEmail({
          to: REPORT_EMAIL,
          subject: '[完了] 統合ワークフロー実行 - 重要案件検出',
          body: summaryMessage
        });
      }
    } catch (mailError) {
      console.error('サマリーメール送信失敗:', mailError);
    }
  }
...
// ========= アプリ統合で業務フロー生成＆通知（UIなし版） =========
/**
 * 業務フロー生成＆通知（パラメータ指定版）
 * @param {string} channelId - 処理対象のチャンネルID（nullの場合は全チャンネル）
 */
function executeWorkflowWithoutUI(channelId = null) {
  console.log('===== 業務フロー生成開始（UIなし） =====');
  console.log(`対象チャンネル: ${channelId || '全チャンネル'}`);
  
  try {
    // 統合ワークフローを実行（UIなしモード）
    const result = executeIntegratedContinuousWorkflow(channelId, true);
    
    console.log('===== 業務フロー生成完了 =====');
    return result;
  } catch (error) {
    console.error('業務フロー生成エラー:', error);
    sendErrorNotification(error, {
      success: false,
      errors: [error.toString()],
      messages: [],
...
// ========= アプリ統合で業務フロー生成＆通知（UIなし版） =========
/**
 * メッセージを取得してワークフローを実行（UIなし版）
 * @param {string} channelId - 対象チャンネルID（nullの場合は全チャンネル）
 */
function getMessagesAsAppWithWorkflow(channelId = null) {
  console.log('===== ワークフロー実行開始 =====');
  console.log(`対象チャンネル: ${channelId || '全参加チャンネル'}`);
  
  // チャンネルIDの検証
  if (channelId && !channelId.startsWith('C')) {
    console.error('エラー: チャンネルIDは「C」で始まる必要があります。');
    sendEmailNotification(
      'ワークフロー実行エラー',
      'チャンネルIDが無効です。チャンネルIDは「C」で始まる必要があります。'
    );
    return;
  }
  
  // UIなし版を呼び出し
  const result = executeWorkflowWithoutUI(channelId);
  
  if (result && result.success) {
    console.log('ワークフロー処理が完了しました。');
  } else {
    console.error('ワークフロー処理でエラーが発生しました。');
  }
  
  return result;
}

// ========= 元のUI版関数（互換性のため残す - 削除済み） =========
// getMessagesAsAppWithWorkflow_Legacy関数は削除されました。
// executeWorkflowWithoutUI関数を使用してください。
...
function sendWorkflowSlackNotification(channelId, data) {
  // チャンネルIDのクリーニング（#や<>を除去）
  const cleanChannelId = channelId.replace(/^[#<]|[>]$/g, '').trim();
  
  console.log(`ワークフロー通知送信: チャンネル ${cleanChannelId}`);
  ...
  try {
    ...
    if (response && response.ok) {
      ...
      console.log(`Slack通知送信完了: ${cleanChannelId}`);
    }
  } catch (error) {
    console.error('Slack通知エラー:', error);
    ...
    // エラーの詳細を記録
    if (error.toString().includes('channel_not_found')) {
      console.error(`チャンネル ${cleanChannelId} が見つかりません`);
      diagnoseNotificationChannel(cleanChannelId);
    }
  }
}
...
// ========= AIを使用した包括的コンプライアンス評価 =========
function evaluateComplianceWithAI(messages, analysisResult) {
  const evaluation = {
    categories: {},
  ...
  const complianceCategories = {
    '個人情報保護': {
      keywords: ['個人情報', 'プライバシー', 'GDPR', '個人データ', '顧客情報'],
      severity: 'HIGH',
      regulations: ['個人情報保護法', 'GDPR', 'プライバシーマーク']
    },
    'インサイダー取引規制': {
      keywords: ['内部情報', 'インサイダー', '未公開情報', '重要事実'],
      severity: 'CRITICAL',
      regulations: ['金融商品取引法', '内部者取引規制']
    },
    '競争法・独占禁止法': {
      keywords: ['独占', '談合', 'カルテル', '下請法', '優越的地位'],
      severity: 'HIGH',
      regulations: ['独占禁止法', '下請法', '景品表示法']
    },
    '労働法令': {
      keywords: ['労働時間', '残業', 'ハラスメント', '労基', '労働基準'],
      severity: 'HIGH',
      regulations: ['労働基準法', '労働安全衛生法', 'パワハラ防止法']
    },
    '知的財産権': {
      keywords: ['特許', '著作権', '商標', '営業秘密', '知財'],
      severity: 'MEDIUM',
      regulations: ['特許法', '著作権法', '商標法', '不正競争防止法']
    },
    '金融規制': {
      keywords: ['金融', '銀行法', '資金決済', 'マネロン', 'AML'],
      severity: 'HIGH',
      regulations: ['銀行法', '資金決済法', '犯罪収益移転防止法']
    },
    '環境規制': {
      keywords: ['環境', '廃棄物', 'CO2', '排出', 'リサイクル'],
      severity: 'MEDIUM',
      regulations: ['環境基本法', '廃棄物処理法', '省エネ法']
    },
    '製品安全・品質': {
      keywords: ['製品安全', '品質', 'リコール', '不具合', 'PL法'],
      severity: 'HIGH',
      regulations: ['製造物責任法', '消費生活用製品安全法', 'JIS規格']
    }
  };
  
  // 各カテゴリの評価を初期化
  Object.keys(complianceCategories).forEach(category => {
  ...
  try {
    // メッセージとアナリシス結果を結合
    const combinedContent = messages.map(m => m.text || '').join('\n') + '\n' + JSON.stringify(analysisResult);
    
    // OpenAI o3を使用した分析
    const aiPrompt = `以下のSlackメッセージと分析結果について、コンプライアンスの観点から評価してください。

対象テキスト:
${combinedContent.substring(0, 3000)}

以下のコンプライアンスカテゴリごとに評価してください:
${Object.entries(complianceCategories).map(([cat, info]) => 
  `- ${cat}: ${info.regulations.join(', ')}`
).join('\n')}

各カテゴリについて以下を判定してください:
1. 該当の有無（該当する/該当しない）
2. リスクレベル（高/中/低）
3. 具体的な懸念事項
4. 推奨される対応策
...
JSONフォーマットで回答してください。`;
    ...
    const aiResponse = callOpenAIAPI(aiPrompt, 'gpt-5');
    const aiEvaluation = parseAIComplianceResponse(aiResponse);
    
    // AI評価結果を統合
    if (aiEvaluation) {
      Object.entries(aiEvaluation).forEach(([category, catEval]) => {
        if (evaluation.categories[category]) {
          evaluation.categories[category] = {
            ...evaluation.categories[category],
            ...catEval,
            aiAnalysis: catEval.analysis || null
          };
          
          // 該当かつ高リスクの場合は critical issues に追加
          if (catEval.applicable && catEval.riskLevel === 'HIGH') {
            evaluation.criticalIssues.push({
              category: category,
              issue: catEval.violations?.[0] || `${category}に関する重大な懸念`,
              recommendation: catEval.recommendations?.[0] || '専門家への相談を推奨'
            });
          }
        }
      });
    ...
    // キーワードベースの追加チェック（AIが見逃した場合のフォールバック）
    Object.entries(complianceCategories).forEach(([category, info]) => {
      const hasKeyword = info.keywords.some(kw => 
        combinedContent.toLowerCase().includes(kw.toLowerCase())
      );
      
      if (hasKeyword && !evaluation.categories[category].applicable) {
        evaluation.categories[category].applicable = true;
        evaluation.categories[category].riskLevel = 'MEDIUM';
        evaluation.categories[category].violations.push(
          `キーワード「${info.keywords.find(kw => combinedContent.toLowerCase().includes(kw.toLowerCase()))}」が検出されました`
        );
        evaluation.categories[category].recommendations.push(
          `${category}の観点から内容を精査することを推奨`
        );
      }
    });
    
    // 全体的なコンプライアンス状態を評価
    const applicableCategories = Object.entries(evaluation.categories)
      .filter(([_, catEval]) => catEval.applicable);
    
    if (applicableCategories.length > 0) {
      const highRiskCount = applicableCategories.filter(([_, catEval]) => 
        catEval.riskLevel === 'HIGH' || catEval.riskLevel === 'CRITICAL'
      ).length;
      
      evaluation.overallCompliance = highRiskCount === 0;
      
      if (highRiskCount > 0) {
        evaluation.recommendations.push(
          '複数の高リスクコンプライアンス項目が検出されました。法務部門への相談を強く推奨します。'
        );
      }
    }
    ...
    if (evaluation.criticalIssues.length > 0) {
      evaluation.recommendations.push(
        `${evaluation.criticalIssues.length}件の重大なコンプライアンス懸念が検出されました。即座の対応が必要です。`
      );
    }
    
  } catch (error) {
    console.error('AIコンプライアンス評価エラー:', error);
    // エラー時はキーワードベースの簡易評価のみ
    evaluation.recommendations.push('AI評価でエラーが発生したため、キーワードベースの評価結果のみを表示しています。');
  }
...
// AIレスポンスのパース
function parseAIComplianceResponse(response) {
  try {
    // JSONレスポンスを抽出
    ...
    // JSONが見つからない場合は構造化されたレスポンスを生成
    const parsed = {};
    const lines = response.split('\n');
    let currentCategory = null;
    
    lines.forEach(line => {
      if (line.includes('該当する') || line.includes('該当しない')) {
        // カテゴリ検出ロジック
      }
    });
...
// ========= 東証開示条件のAI判定 =========
function evaluateTSEDisclosureRequirement(messages, analysisResult) {
  const evaluation = {
    requiresDisclosure: false,
    disclosureType: null,
    urgency: 'LOW',
  ...
  try {
    ...
    // 東証の適時開示基準に基づくプロンプト
    const aiPrompt = `以下のSlackメッセージと分析結果について、東京証券取引所（東証）の適時開示制度の観点から評価してください。
...
以下の観点で判定してください：
1. 適時開示の必要性（必要/不要）
2. 該当する開示事項の種類（決定事実/発生事実/決算情報/その他）
3. 緊急度（高/中/低）
4. 具体的な該当項目
5. 開示が必要な理由
6. 推奨される対応
...
JSONフォーマットで回答してください。
{
  ...
  "disclosureType": "決定事実/発生事実/決算情報/その他",
  "urgency": "HIGH/MEDIUM/LOW",
  "specificItems": ["該当する具体的な開示項目"],
  "reasons": ["開示が必要な理由"],
  "recommendedActions": ["推奨される対応"],
  "analysis": "総合的な分析コメント"
}`;
    
    // o3モデルを使用してより精度の高い判定を行う
    const aiResponse = callOpenAIAPI(aiPrompt, 'gpt-5');
    const aiResult = parseTSEDisclosureResponse(aiResponse);
    
    if (aiResult) {
      evaluation.requiresDisclosure = aiResult.requiresDisclosure || false;
      evaluation.disclosureType = aiResult.disclosureType || null;
      evaluation.urgency = aiResult.urgency || 'LOW';
      evaluation.specificItems = aiResult.specificItems || [];
      evaluation.reasons = aiResult.reasons || [];
      evaluation.recommendedActions = aiResult.recommendedActions || [];
      evaluation.aiAnalysis = aiResult.analysis || null;
    }
    
    // キーワードベースのフォールバックチェック
    const disclosureKeywords = {
      '決定事実': ['M&A', '合併', '買収', '増資', '減資', '業務提携', '資本提携', '新株', '配当'],
      '発生事実': ['災害', '事故', '訴訟', '行政処分', '倒産', '不渡り'],
      '決算情報': ['業績予想', '修正', '決算', '配当予想', '30%', '大幅']
    };
    ...
    Object.entries(disclosureKeywords).forEach(([type, keywords]) => {
      keywords.forEach(keyword => {
        if (combinedContent.includes(keyword)) {
          keywordMatches.push({ type, keyword });
        }
      });
    });
    
    // AI判定がなくてもキーワードが検出された場合
    if (!evaluation.requiresDisclosure && keywordMatches.length > 0) {
      evaluation.requiresDisclosure = true;
      evaluation.disclosureType = keywordMatches[0].type;
      evaluation.urgency = 'MEDIUM';
      evaluation.specificItems.push(`キーワード「${keywordMatches[0].keyword}」が検出されました`);
      evaluation.reasons.push('開示に関連するキーワードが含まれているため、詳細な検討が必要です');
      evaluation.recommendedActions.push('IR部門および法務部門に相談してください');
    }
    
    // 緊急度が高い場合の追加アクション
    if (evaluation.urgency === 'HIGH' && evaluation.requiresDisclosure) {
      evaluation.recommendedActions.unshift('【緊急】直ちにIR部門に連絡し、開示準備を開始してください');
    }
    
  } catch (error) {
    console.error('東証開示条件評価エラー:', error);
    evaluation.recommendedActions.push('AI判定でエラーが発生しました。IR部門に直接確認してください');
  }
...
// 東証開示レスポンスのパース
function parseTSEDisclosureResponse(response) {
  try {
    // JSONレスポンスを抽出
    const jsonMatch = response.match(/\{[\s\S]*\}/);
    if (jsonMatch) {
    ...
    console.error('東証開示レスポンスのパースエラー:', error);
    return null;
  }
}
...
/**
 * エラー通知を管理者に送信（旧バージョン）
 * @deprecated sendErrorNotification(error, workflowResult) を使用してください
 */
function sendAdminErrorNotification(error) {
...
function createDetailedWorkflowSpreadsheet(analysisResult, governanceResult, messages) {
  ...
  if (governanceResult.riskReasons && governanceResult.riskReasons.length > 0) {
    summarySheet.getRange(currentRow, 1).setValue('リスク評価の根拠');
    summarySheet.getRange(currentRow, 1).setFontWeight('bold');
    summarySheet.getRange(currentRow, 1).setBackground('#ffebee');
    currentRow++;
    
    governanceResult.riskReasons.forEach((reason, index) => {
      summarySheet.getRange(currentRow, 1).setValue(`${index + 1}. ${reason}`);
  ...
  // コンプライアンス評価を追加
  if (governanceResult.complianceEvaluation) {
    summarySheet.getRange(currentRow, 1).setValue('コンプライアンス評価');
    summarySheet.getRange(currentRow, 1).setFontWeight('bold');
    summarySheet.getRange(currentRow, 1).setBackground('#e3f2fd');
    currentRow++;
    
    // 該当するカテゴリを表示
    const applicableCategories = Object.entries(governanceResult.complianceEvaluation.categories || {})
      .filter(([_, catEval]) => catEval.applicable);
    
    if (applicableCategories.length > 0) {
      applicableCategories.forEach(([category, catEval]) => {
        summarySheet.getRange(currentRow, 1).setValue(`● ${category}`);
        summarySheet.getRange(currentRow, 2).setValue(`リスク: ${catEval.riskLevel}`);
        
        // リスクレベルに応じた色分け
        if (catEval.riskLevel === 'HIGH' || catEval.riskLevel === 'CRITICAL') {
          summarySheet.getRange(currentRow, 1, 1, 2).setBackground('#ffcdd2');
        } else if (catEval.riskLevel === 'MEDIUM') {
          summarySheet.getRange(currentRow, 1, 1, 2).setBackground('#fff9c4');
        }
        currentRow++;
        
        // AI分析結果がある場合
        if (catEval.aiAnalysis) {
          summarySheet.getRange(currentRow, 1).setValue(`  AI判定: ${catEval.aiAnalysis}`);
          summarySheet.getRange(currentRow, 1, 1, 2).merge();
          currentRow++;
        }
        
        // 違反事項がある場合
        if (catEval.violations && catEval.violations.length > 0) {
          catEval.violations.forEach(violation => {
            summarySheet.getRange(currentRow, 1).setValue(`  懸念: ${violation}`);
            summarySheet.getRange(currentRow, 1, 1, 2).merge();
            summarySheet.getRange(currentRow, 1, 1, 2).setFontStyle('italic');
            currentRow++;
          });
        }
        
        // 推奨事項がある場合
        if (catEval.recommendations && catEval.recommendations.length > 0) {
          catEval.recommendations.forEach(rec => {
            summarySheet.getRange(currentRow, 1).setValue(`  → ${rec}`);
            summarySheet.getRange(currentRow, 1, 1, 2).merge();
            summarySheet.getRange(currentRow, 1, 1, 2).setFontColor('#1976d2');
            currentRow++;
          });
        }
        currentRow++;
      ...
      summarySheet.getRange(currentRow, 1).setValue('該当するコンプライアンスリスクは検出されませんでした');
      summarySheet.getRange(currentRow, 1, 1, 2).merge();
      currentRow++;
    }
    
    // 総合的な推奨事項
    if (governanceResult.complianceEvaluation.recommendations && governanceResult.complianceEvaluation.recommendations.length > 0) {
      currentRow++;
      summarySheet.getRange(currentRow, 1).setValue('【総合推奨事項】');
      summarySheet.getRange(currentRow, 1).setFontWeight('bold');
      currentRow++;
      
      governanceResult.complianceEvaluation.recommendations.forEach(rec => {
        summarySheet.getRange(currentRow, 1).setValue(`• ${rec}`);
        summarySheet.getRange(currentRow, 1, 1, 2).merge();
        currentRow++;
      });
  ...
  // 東証開示条件評価を追加
  if (governanceResult.tseDisclosureEvaluation) {
    currentRow++;
    summarySheet.getRange(currentRow, 1).setValue('東証開示条件評価');
    summarySheet.getRange(currentRow, 1).setFontWeight('bold');
    summarySheet.getRange(currentRow, 1).setBackground('#ffe0b2');
    currentRow++;
    
    const tseEval = governanceResult.tseDisclosureEvaluation;
    
    summarySheet.getRange(currentRow, 1).setValue('開示要否');
    summarySheet.getRange(currentRow, 2).setValue(tseEval.requiresDisclosure ? '要' : '不要');
    if (tseEval.requiresDisclosure) {
      summarySheet.getRange(currentRow, 2).setBackground('#ffcdd2');
    }
    currentRow++;
    
    if (tseEval.requiresDisclosure) {
      summarySheet.getRange(currentRow, 1).setValue('開示種別');
      summarySheet.getRange(currentRow, 2).setValue(tseEval.disclosureType || '未分類');
      currentRow++;
      
      summarySheet.getRange(currentRow, 1).setValue('緊急度');
      summarySheet.getRange(currentRow, 2).setValue(tseEval.urgency || 'LOW');
      if (tseEval.urgency === 'HIGH') {
        summarySheet.getRange(currentRow, 2).setBackground('#ffcdd2');
      } else if (tseEval.urgency === 'MEDIUM') {
        summarySheet.getRange(currentRow, 2).setBackground('#fff9c4');
      }
      currentRow++;
      
      if (tseEval.specificItems && tseEval.specificItems.length > 0) {
        summarySheet.getRange(currentRow, 1).setValue('該当項目');
        currentRow++; tseEval.specificItems.forEach(item => {
          summarySheet.getRange(currentRow, 1).setValue(`・ ${item}`);
          summarySheet.getRange(currentRow, 1, 1, 2).merge();
          currentRow++;
        });
      }
      
      if (tseEval.reasons && tseEval.reasons.length > 0) {
        summarySheet.getRange(currentRow, 1).setValue('開示理由');
        currentRow++;
        tseEval.reasons.forEach(reason => {
          summarySheet.getRange(currentRow, 1).setValue(`・ ${reason}`);
          summarySheet.getRange(currentRow, 1, 1, 2).merge();
          currentRow++;
        });
      }
      
      if (tseEval.recommendedActions && tseEval.recommendedActions.length > 0) {
        summarySheet.getRange(currentRow, 1).setValue('推奨アクション');
        currentRow++;
        tseEval.recommendedActions.forEach(action => {
          summarySheet.getRange(currentRow, 1).setValue(`→ ${action}`);
          summarySheet.getRange(currentRow, 1, 1, 2).merge();
          summarySheet.getRange(currentRow, 1, 1, 2).setFontColor('#d32f2f');
          currentRow++;
        });
      }
    }
    
    if (tseEval.aiAnalysis) {
      currentRow++;
      summarySheet.getRange(currentRow, 1).setValue('AI分析結果');
      summarySheet.getRange(currentRow, 1).setFontWeight('bold');
      currentRow++;
      summarySheet.getRange(currentRow, 1).setValue(tseEval.aiAnalysis);
      summarySheet.getRange(currentRow, 1, 1, 2).merge();
      summarySheet.getRange(currentRow, 1).setWrap(true);
      currentRow++;
    }
  }
...
function sendEnhancedHtmlEmail(workflowResult) {
  ...
  const spreadsheetUrl = workflowResult.spreadsheetUrl; // デフォルトURLを使用しない
  const effectiveRisk = workflowResult.governance?.riskLevel || 'LOW';
  
  // 元メッセージのSlackリンク（最大5件、重複スレッドは除外）
  const sourceLinks = (() => {
    const seen = new Set();
    const links = [];
    const msgs = Array.isArray(workflowResult.messages) ? workflowResult.messages : [];
    for (let i = 0; i < msgs.length && links.length < 5; i++) {
      const msg = msgs[i] || {};
      const channelId = msg.channelId || msg.channel_id || '';
      const ts = msg.ts || msg.message_ts || '';
      const threadTs = msg.thread_ts || msg.threadTs || '';
      const link = buildSlackPermalink(channelId, ts, threadTs);
      if (link) {
        const key = threadTs && threadTs !== ts ? `${channelId}-${threadTs}` : `${channelId}-${ts}`;
        if (!seen.has(key)) {
          seen.add(key);
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p style="margin:6px 0 0 0;">生成日時: ${new Date().toLocaleString('ja-JP')}</p>
      ${spreadsheetUrl ? `<p style="margin:8px 0 0 0; word-break: break-all;">📑 スプレッドシート: <a href="${spreadsheetUrl}" style="color:#60A5FA; text-decoration:underline;">${spreadsheetUrl}</a></p>` : ''}
    </div>
    
    <table role="presentation" width="100%" style="border-collapse: separate; border-spacing: 12px 12px; margin: 0 0 20px 0;">
      <tr>
        <td width="50%" style="padding: 0;">
          <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:8px; padding:16px; text-align:center;">
            <div style="font-size:28px; font-weight:700; color:#111827; line-height:1;">${workflowResult.messages?.length || 0}</div>
            <div style="font-size:13px; color:#6b7280; margin-top:6px;">分析メッセージ数</div>
          </div>
        </td>
        <td width="50%" style="padding: 0;">
          <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:8px; padding:16px; text-align:center;">
            <div style="font-size:28px; font-weight:700; color:#111827; line-height:1;">${workflowResult.analysis?.topics?.length || 0}</div>
            <div style="font-size:13px; color:#6b7280; margin-top:6px;">抽出議題数</div>
          </div>
        </td>
      </tr>
      <tr>
        <td width="50%" style="padding: 0;">
          <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:8px; padding:16px; text-align:center;">
            <div style="font-size:28px; font-weight:700; color:#111827; line-height:1;">${workflowResult.analysis?.actionItems?.length || 0}</div>
            <div style="font-size:13px; color:#6b7280; margin-top:6px;">アクション項目</div>
          </div>
        </td>
        <td width="50%" style="padding: 0;">
          <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:8px; padding:16px; text-align:center;">
            <div style="font-size:22px; font-weight:700; color:#111827; line-height:1;">${effectiveRisk}</div>
            <div style="font-size:13px; color:#6b7280; margin-top:6px;">リスクレベル</div>
          </div>
        </td>
      </tr>
    </table>
    ...
    <div class="content">
      ...
      <div class="action-items">
        ...
        <table role="presentation" width="100%" style="border-collapse: separate; border-spacing: 0 8px;">
          ${workflowResult.analysis.actionItems.slice(0, 5).map(item => `
            <tr>
              <td style="background:#ffffff; border:1px solid #FCD34D; border-left:3px solid #F59E0B; border-radius:6px; padding:10px; font-size:14px; color:#374151;">
                ${typeof item === 'object' ? (item.task || item.action || item) : item}
                ${typeof item === 'object' && item.owner ? ` <span style='color:#6b7280'>(担当: ${item.owner})</span>` : ''}
                ${typeof item === 'object' && item.deadline ? ` <span style='color:#6b7280'>- 期限: ${item.deadline}</span>` : ''}
              </td>
            </tr>
          `).join('')}
        </table>
      ...
      <div class="governance-alert">
        ...
        ${workflowResult.governance.riskReasons && workflowResult.governance.riskReasons.length > 0 ? `
        <div style="margin-top: 10px; padding: 10px; background-color: #f9f9f9; border-left: 3px solid ${priorityColor};">
          <p style="font-weight: bold; margin-bottom: 5px;">リスク評価の根拠：</p>
          <ul style="margin: 0; padding-left: 20px;">
            ${workflowResult.governance.riskReasons.map(reason => `<li>${reason}</li>`).join('')}
          </ul>
        </div>
        ` : ''}
      ...
      ${workflowResult.governance?.complianceEvaluation ? `
      <div class="section" style="margin-top: 20px; padding: 15px; background-color: #e3f2fd; border-radius: 8px;">
        <h3>🔍 コンプライアンス評価</h3>
        ${(() => {
          const applicableCategories = Object.entries(workflowResult.governance.complianceEvaluation.categories || {})
            .filter(([_, catEval]) => catEval.applicable);
          
          if (applicableCategories.length > 0) {
            return applicableCategories.map(([category, catEval]) => `
              <div style="margin-bottom: 10px; padding: 10px; background-color: white; border-radius: 5px; border-left: 4px solid ${
                catEval.riskLevel === 'HIGH' || catEval.riskLevel === 'CRITICAL' ? '#f44336' : 
                catEval.riskLevel === 'MEDIUM' ? '#ff9800' : '#4caf50'
              };">
                <h4 style="margin: 0 0 5px 0; color: #333;">${category}</h4>
                <p style="margin: 0; font-size: 14px;"><strong>リスクレベル:</strong> ${catEval.riskLevel}</p>
                ${catEval.aiAnalysis ? `<p style="margin: 5px 0; font-size: 13px; color: #666;"><strong>AI判定:</strong> ${catEval.aiAnalysis}</p>` : ''}
                ${catEval.violations && catEval.violations.length > 0 ? `
                  <div style="margin-top: 5px;">
                    <p style="margin: 0; font-size: 13px; color: #d32f2f;">懸念事項:</p>
                    <ul style="margin: 5px 0; padding-left: 20px;">
                      ${catEval.violations.map(v => `<li style="font-size: 13px; color: #666;">${v}</li>`).join('')}
                    </ul>
                  </div>
                ` : ''}
                ${catEval.recommendations && catEval.recommendations.length > 0 ? `
                  <div style="margin-top: 5px;">
                    <p style="margin: 0; font-size: 13px; color: #1976d2;">推奨事項:</p>
                    <ul style="margin: 5px 0; padding-left: 20px;">
                      ${catEval.recommendations.map(r => `<li style="font-size: 13px; color: #666;">${r}</li>`).join('')}
                    </ul>
                  </div>
                ` : ''}
              </div>
            ...
            return '<p style="color: #4caf50;">該当するコンプライアンスリスクは検出されませんでした。</p>';
          }
        })()}
        ${workflowResult.governance.complianceEvaluation.recommendations && workflowResult.governance.complianceEvaluation.recommendations.length > 0 ? `
          <div style="margin-top: 15px; padding: 10px; background-color: #fff3cd; border-radius: 5px;">
            <p style="margin: 0 0 5px 0; font-weight: bold; color: #856404;">総合推奨事項:</p>
            <ul style="margin: 5px 0; padding-left: 20px;">
              ${workflowResult.governance.complianceEvaluation.recommendations.map(r => `<li style="color: #856404;">${r}</li>`).join('')}
            </ul>
          </div>
        ` : ''}
      </div>
    ...
    <div class="section" style="margin-top: 20px;">
      <h3>📎 元となったSlackメッセージ</h3>
      <ul style="margin: 8px 0; padding-left: 20px;">
        ${sourceLinks.map((s, i) => `
          <li><a href="${s.link}" target="_blank" style="color:#1264a3; text-decoration:none;">関連メッセージ ${i + 1}（${s.channelId}）を開く</a></li>
        `).join('')}
      </ul>
    </div>
    ` : ''}
    ...
    <div class="footer" style="font-size:12px; line-height:1.6;">
      <p style="margin:0;">このメールは自動生成されました | Slackガバナンスシステム v2.0</p>
      <p style="margin:6px 0 0 0;">© 2024 Automated Workflow System</p>
    </div>
  </div>
</body>
</html>`;
...
【元メッセージURL】
${sourceLinks.length > 0 ? sourceLinks.map((s, i) => `- 関連メッセージ ${i + 1}: ${s.link}`).join('\n') : '（リンクなし）'}
...
function sendEnhancedSlackNotification(workflowResult) {
  ...
  // 元メッセージのSlackリンク（最大3件）
  const linkItems = (() => {
    const seen = new Set();
    const items = [];
    const msgs = Array.isArray(workflowResult.messages) ? workflowResult.messages : [];
    for (let i = 0; i < msgs.length && items.length < 3; i++) {
      const msg = msgs[i] || {};
      const channelId = msg.channelId || msg.channel_id || '';
      const ts = msg.ts || msg.message_ts || '';
      const threadTs = msg.thread_ts || msg.threadTs || '';
  ...
  if (linkItems.length > 0) {
    blocks.push({
      ...
      text: {
        type: 'mrkdwn',
        text: `*📎 元メッセージ:*\n${linkItems.map((l, i) => `• <${l}|関連メッセージ ${i + 1}>`).join('\n')}`
      }
    });
    blocks.push({ type: 'divider' });
  }
  ...
  // ガバナンス警告
  if (workflowResult.governance?.requiresAction) {
    blocks.push({
      type: "section",
      text: {
        type: "mrkdwn",
  ...
  try {
    // チャンネルIDのクリーニング（#や<>を除去）
    const cleanChannelId = notifyChannel.replace(/^[#<]|[>]$/g, '').trim();
    ...
    // エラーの詳細を記録
    if (e.toString().includes('channel_not_found')) {
      const cleanChannelId = notifyChannel.replace(/^[#<]|[>]$/g, '').trim();
      console.error(`チャンネル ${cleanChannelId} が見つかりません`);
      diagnoseNotificationChannel(cleanChannelId);
    }
...
function fetchAllJoinedChannelsMessages() {
  ...
  if (!channels || channels.length === 0) {
    console.warn('警告: 取得可能なチャンネルがありません');
    return [];
  }
  ...
  console.log(`${channels.length}個のチャンネルからメッセージを取得中...`);

  channels.forEach(channel => {
    try {
      console.log(`チャンネル #${channel.name} からメッセージ取得中...`);
      ...
      if (messages && messages.length > 0) {
        ...
        console.log(`  ℹ️ メッセージなし`);
      }
    } catch (error) {
      errorCount++;
      console.error(`  ❌ チャンネル #${channel.name} のエラー:`, error.toString());
    }
  });

  console.log(`メッセージ取得完了: 成功=${successCount}, エラー=${errorCount}, 合計メッセージ数=${allMessages.length}`);
...
function getJoinedChannels() {
  ...
  try {
    ...
    if (!publicResponse || !publicResponse.ok) {
      console.error('パブリックチャンネル取得失敗:', publicResponse?.error || 'Unknown error');
      return allChannels;
    }
    ...
    console.error('パブリックチャンネル取得エラー:', e.toString());
  }
  ...
  try {
    ...
    if (privateResponse && privateResponse.ok) {
    ...
    if (e.toString().includes('missing_scope')) {
      ...
      console.error('プライベートチャンネル取得エラー:', e.toString());
    }
  }
  ...
  if (allChannels.length === 0) {
    console.warn('警告: Botが参加しているチャンネルが見つかりません。Botをチャンネルに招待してください。');
  }
...
function callOpenAIAPI(prompt, model = 'gpt-5') {
  ...
  // o3系モデルは基本的にv1/responsesの使用が推奨のため、まずはこちらを試す
  if (false) {
    try {
      const content = callOpenAIAPIResponsesSimple(prompt, model);
      if (content && typeof content === 'string' && content.trim() !== '') {
        return content;
      }
      console.warn('o3レスポンスが空だったためgpt-5へフォールバックします');
      // 空レスポンスの場合はgpt-5にフォールバック
      return callOpenAIAPI(prompt, 'gpt-5');
    } catch (e) {
      console.warn('o3レスポンス取得に失敗したためgpt-5へフォールバックします:', e.toString());
      return callOpenAIAPI(prompt, 'gpt-5');
    }
  }

  const url = OPENAI_URL_RESPONSES || 'https://api.openai.com/v1/responses';
  
  const input = `SYSTEM:\nあなたはコンプライアンスと法規制の専門家です。与えられた情報を正確に分析し、必ず指定されたJSONフォーマットで回答してください。\n\nUSER:\n${prompt}`;
  const payload = {
    model: model,
    input: input,
    max_output_tokens: 4000
  ...
  // o3モデルの特別な処理
  if (false) {
    delete payload.temperature; // o3はtemperature=1のみサポート
    payload.max_output_tokens = 4000; // 途切れ防止のため増加
  ...
  try {
    ...
    if (status !== 200) {
      ...
      try {
        ...
        console.error('OpenAI API Error (raw body):', bodyText.substring(0, 500));
      }
    ...
    const content = extractTextFromOpenAIResponse_(data);
    if (!content || (typeof content === 'string' && content.trim() === '')) {
      console.warn('Chat Completionsから空のコンテンツが返されました');
      if (model !== 'gpt-5') {
        console.warn('gpt-5へフォールバックします');
        return callOpenAIAPI(prompt, 'gpt-5');
      }
...
// v1/responses向けのシンプル呼び出し（o3系モデル用）
function callOpenAIAPIResponsesSimple(prompt, model) {
  const apiKey = PropertiesService.getScriptProperties().getProperty('OPENAI_API_KEY');
  if (!apiKey) {
    throw new Error('OpenAI APIキーが設定されていません。スクリプトプロパティに OPENAI_API_KEY を設定してください。');
  }
  ...
  const url = OPENAI_URL_RESPONSES || 'https://api.openai.com/v1/responses';
  // Responses APIではmessagesではなくinputを使用
  const combinedInput = 'あなたはコンプライアンスと法規制の専門家です。与えられた情報を正確に分析し、必ず指定されたJSONフォーマットで回答してください。\n\n' + String(prompt);
  ...
  // gpt-5用設定（Responses API）
  payload.max_output_tokens = 4000;  // JSONが切れないよう増加

  // JSON出力を促す（Responses APIの text.format を使用）
  payload.text = {
    format: {
      type: 'json_schema',
      name: 'analysis',
  ...
  // o3は時間がかかるため、十分なリトライ回数/遅延で実行
  const isO3Simple = false;
  const response = retryWithBackoff(() => UrlFetchApp.fetch(url, options), null, null);
  const status = response.getResponseCode();
  const bodyText = response.getContentText();
  ...
  if (status !== 200) {
    ...
    try {
      ...
      console.error('OpenAI Responses API Error (raw body):', bodyText.substring(0, 500));
    }
    throw new Error(`OpenAI Responses API error ${status}: ${errorMessage}`);
  }
  ...
  try {
    ...
    console.error('Responses API JSON parse error:', e.toString());
    console.error('Responses API body (first 500 chars):', bodyText.substring(0, 500));
    throw e;
  }

  // Structured Outputs/テキストの多様な戻り形式に対応
  try {
    const scanOutput = (obj) => {
      if (!obj) return '';
      const outputs = Array.isArray(obj.output) ? obj.output : [];
      for (const out of outputs) {
        const contents = Array.isArray(out.content) ? out.content : [];
        for (const part of contents) {
          // 既知タイプ: output_text / output_json / output_structured
          if (part && typeof part === 'object') {
            ...
            if (part.type === 'output_structured') {
              const v = part.parsed || part.structured || part.json;
              if (v !== undefined) {
            ...
            if (part.parsed !== undefined) {
              return typeof part.parsed === 'string' ? part.parsed : JSON.stringify(part.parsed);
            }
    ...
    const scanned = scanOutput(data) || (data.response ? scanOutput(data.response) : '');
    if (typeof scanned === 'string' && scanned.trim() !== '') {
      return scanned;
    }
  } catch (_) {/* ignore and continue */}

  // Responses APIの推奨: output_text が存在する
  if (typeof data.output_text === 'string' && data.output_text.trim() !== '') {
    return data.output_text;
  }
  ...
  // 後方互換のためにchoices経由も試す
  const legacyContent = data && data.choices && data.choices[0] && data.choices[0].message ? data.choices[0].message.content : '';
  ...
  console.warn('Responses APIから空のコンテンツが返されました');
  return '';
}
...
function testPrivateChannelAccess() {
  ...
  privateChannels.forEach(channel => {
    ...
    try {
      ...
      if (info.ok && info.channel) {
        ...
        console.log(`❌ チャンネル情報取得失敗: ${info.error || '不明なエラー'}`);
        results.push({
          channel: channel.name,
          id: channel.id,
          is_member: false,
    ...
    try {
      ...
      if (historyResult.ok) {
        ...
        lastResult.canReadMessages = true;
      } else {
        console.log(`❌ メッセージ履歴取得失敗: ${historyResult.error || '不明なエラー'}`);
        const lastResult = results[results.length - 1];
        lastResult.canReadMessages = false;
        lastResult.historyError = historyResult.error;
      }
    } catch (error) {
      console.log(`❌ 履歴取得エラー: ${error.toString()}`);
      const lastResult = results[results.length - 1];
      lastResult.canReadMessages = false;
      lastResult.historyError = error.toString();
    }
  });
  ...
  console.log(`メンバーのチャンネル数: ${results.filter(r => r.is_member).length}`);
  console.log(`アクセス可能なチャンネル数: ${results.filter(r => r.canReadMessages).length}`);
  
  const needsInvite = results.filter(r => !r.is_member);
  if (needsInvite.length > 0) {
    console.log('\n招待が必要なチャンネル:');
    needsInvite.forEach(r => {
      console.log(`  - #${r.channel} (${r.id})`);
    });
  }
...
// トピック配列から重要度の高いものを抽出（存在しない場合はそのまま返す）
function filterCriticalTopics(topics, overallPriority) {
  try {
    ...
    const normalize = (p) => {
      ...
      if (s.includes('LOW') || s.includes('低')) return 1;
      return 0;
    };
    const threshold = 3; // HIGH以上
    const filtered = topics.filter(t => normalize(t.priority) >= threshold);
    if (filtered.length > 0) return filtered;
    // フィルタでゼロになった場合は元を返す（情報損失回避）
    return topics;
  } catch (_) {
    return topics || [];
  }
...
/**
 * 通知チャンネルの診断
 * @param {string} channelId - 診断するチャンネルID
 */
function diagnoseNotificationChannel(channelId) {
  console.log(`=== 通知チャンネル診断開始: ${channelId} ===`);
  
  try {
    // 1. Botが参加しているチャンネル一覧を取得
    const joinedChannels = getAllJoinedChannels();
    const isJoined = joinedChannels.some(ch => ch.id === channelId);
    
    if (!isJoined) {
      console.error(`❌ Bot はチャンネル ${channelId} に参加していません`);
      console.log('解決方法:');
      console.log('1. Slackでチャンネルを開く');
      console.log('2. /invite @[bot-name] コマンドを実行');
    ...
    // 2. チャンネル情報を取得
    const channelInfo = slackAPI('conversations.info', { channel: channelId });
    console.log(`✅ チャンネル情報取得成功: ${channelInfo.channel.name}`);
    console.log(`- Private: ${channelInfo.channel.is_private}`);
    console.log(`- Archived: ${channelInfo.channel.is_archived}`);
    
    if (channelInfo.channel.is_archived) {
      console.error('⚠️ チャンネルがアーカイブされています');
      return false;
    }
    
    // 3. テストメッセージ送信を試みる
    console.log('テストメッセージ送信中...');
    const testResponse = slackAPI('chat.postMessage', {
      channel: channelId,
      text: 'テスト: 通知チャンネル設定確認'
    });
    
    if (testResponse.ok) {
      console.log('✅ メッセージ送信成功！通知チャンネルは正常に動作します');
      return true;
    }
