タイトル: BILLING_SERVICE（BILLING_VENDOR）請求データ マッピング指針 v0.1
...
2) 請求ヘッダ（Invoice）
- customer_id: WAREHOUSE_SYS.customer_id（BILLING_SERVICEの顧客コード）
- invoice_number: WAREHOUSE_SYS採番（YYYYMM-案件ID-枝番など）
- issue_date: 発行日（承認日/当月末 いずれかポリシー）
- billing_period_from / to: 請求対象期間
- due_date: 支払期日（顧客与信/支払サイトから計算）
- bill_to_name/address/email: BILLING_SERVICE側マスター同期値（必要時は上書き禁止）
- currency: JPY
- tax_mode / tax_total / total_excl_tax / total_incl_tax: WAREHOUSE_SYS計算結果
...
5) マッピングルール
- ヘッダ顧客/請求先: 原則BILLING_SERVICEマスターを真実源。WAREHOUSE_SYSはcustomer_idのみ送る。
- 税/端数: 行→請求書の順に丸め。BILLING_SERVICE側の自動税計算と二重計算にならないよう、金額を明示送信。
...
7) 受信（入金/ステータス）
- invoice_id/token: BILLING_SERVICEの発行IDをWAREHOUSE_SYSに保存
- status: 発行/送付/入金/遅延 など
- payment: 入金日/入金額/手数料をWAREHOUSE_SYSへ同期→消込

8) CSVフォールバック（BILLING_SERVICEのCSV仕様を前提に雛形）
ヘッダ:
- 顧客コード, 請求書番号, 発行日, 請求期間自, 請求期間至, 支払期日, 通貨, 税区分, 備考
明細:
- 行番号, 品目名, 数量, 単位, 単価, 金額(税抜), 税率, 備考

9) フィールドソース一覧（WAREHOUSE_SYS→BILLING_SERVICE）
- 顧客コード: customer_id
