...
目的: 請求計算をWAREHOUSE_SYSで一元化するため、案件単位の料金・算定ルール・実績入力項目を標準化する。
...
- 請求ヘッダ/明細(Invoice/InvoiceLine): 締め計算の結果。BILLING_SERVICE送信のソース。

2) Project（案件）主要項目
- project_id: 文字列（主キー、HubSpot案件IDと同一）
- customer_id: 文字列（BILLING_SERVICE/MFK顧客IDと同一）
- warehouse_id: 文字列
...
- 差異照合: 外部実績合計とWAREHOUSE_SYS計算の金額比較（行粒度はPhase2）
