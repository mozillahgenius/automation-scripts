...
function createSampleFlowData() {
  ...
  flowSheet.setColumnWidth(5, 120); // プロセスオーナー
  flowSheet.setColumnWidth(6, 250); // 作業内容
  flowSheet.setColumnWidth(7, 150); // 条件分岐
  flowSheet.setColumnWidth(8, 120); // 利用ツール
  flowSheet.setColumnWidth(9, 200); // URLリンク
  flowSheet.setColumnWidth(10, 200); // 備考
...
function writeFlowToSheetWithGovernance(sheet, flowRows, governanceCheck) {
      ...
      if (typeof row === 'object' && row !== null) {
        ...
        for (let j = 0; j < actions.length; j++) {
          ...
          for (const header of headers) {
            ...
            } else if (header === 'CEAVOP対応') {
              value = checkCEAVOPControl(processName, actions[j], condition, dept);
            } else if (header === 'プロセスオーナー') {
              // プロセスオーナーは担当役割からRACIの'A'を抽出
              value = j === 0 ? extractProcessOwner(row['担当役割'] || '') : '';
  ...
  sheet.setColumnWidth(5, 120); // プロセスオーナー
  sheet.setColumnWidth(6, 300); // 作業内容
  sheet.setColumnWidth(7, 150); // 条件分岐
  sheet.setColumnWidth(8, 150); // 利用ツール
  sheet.setColumnWidth(9, 200); // URLリンク
  sheet.setColumnWidth(10, 200); // 備考
  sheet.setColumnWidth(11, 250); // 法令・規制
  sheet.setColumnWidth(12, 250); // 内部統制
  sheet.setColumnWidth(13, 200); // CEAVOP対応
  sheet.setColumnWidth(14, 300); // コンプライアンス留意点
...
function writeWorkSpec(workSpec) {
  ...
  const rowData = [
    ...
    extractDeliverableCreators(workSpec.deliverables, workSpec.raci),  // 成果物作成者
    extractDeliverableApprovers(workSpec.deliverables, workSpec.approvals, workSpec.raci),  // 成果物承認者
    extractDeliverableTimeline(workSpec.deliverables, workSpec.timeline),  // 作成日/承認日
    formatArray(workSpec.org_structure),
    extractProcessOwnerFromWorkSpec(workSpec.raci),  // プロセスオーナー
    ...
    extractCEAVOPFromWorkSpec(workSpec),  // CEAVOP対応
...
function createWorkSpecSheet() {
  ...
  const headers = [
    ...
    '成果物作成者',  // NEW: 各成果物の作成者
    '成果物承認者',  // NEW: 各成果物の承認者
    '作成日/承認日',  // NEW: 作成・承認のタイムライン
    '体制',
    'プロセスオーナー',  // NEW: プロセス全体責任者
    'RACI',
    'スケジュール',
    '要件・制約',
    'リスク・対策',
...
function createFlowSheet(sheetName) {
  ...
  sh.setColumnWidth(5, 120); // プロセスオーナー
  sh.setColumnWidth(6, 300); // 作業内容
  sh.setColumnWidth(7, 150); // 条件分岐
  sh.setColumnWidth(8, 150); // 利用ツール
  sh.setColumnWidth(9, 200); // URLリンク
  sh.setColumnWidth(10, 200); // 備考
  sh.setColumnWidth(11, 250); // 法令・規制
  sh.setColumnWidth(12, 250); // 内部統制
  sh.setColumnWidth(13, 200); // CEAVOP対応
  sh.setColumnWidth(14, 300); // コンプライアンス留意点
...
function createFlowSheet(sheetName) {
  ...
  sh.setColumnWidth(5, 100); // プロセスオーナー
  sh.setColumnWidth(6, 250); // 作業内容
  sh.setColumnWidth(7, 150); // 条件分岐
  sh.setColumnWidth(8, 120); // 利用ツール
  sh.setColumnWidth(9, 150); // URLリンク
  sh.setColumnWidth(10, 200); // 備考
  sh.setColumnWidth(11, 200); // 法令・規制
  sh.setColumnWidth(12, 200); // 内部統制
  sh.setColumnWidth(13, 200); // CEAVOP対応
  sh.setColumnWidth(14, 250); // コンプライアンス留意点
...
// 成果物作成者を抽出する関数
function extractDeliverableCreators(deliverables, raci) {
  ...
  if (Array.isArray(deliverables)) {
    deliverables.forEach(deliverable => {
      // RACI情報から作成者を特定
      if (raci && typeof raci === 'object') {
        const responsible = raci.responsible || raci.R || '';
        if (responsible) {
          creators.push(`${deliverable}: ${responsible}`);
        }
      }
    });
  }
...
// 成果物承認者を抽出する関数
function extractDeliverableApprovers(deliverables, approvals, raci) {
  if (!deliverables) return '';
  ...
  if (Array.isArray(approvals)) {
    approvals.forEach(approval => {
      if (approval && approval.includes('承認')) {
        approvers.push(approval);
      }
    });
  }

  // RACIからAccountableを抽出
  if (raci && typeof raci === 'object') {
    const accountable = raci.accountable || raci.A || '';
    if (accountable && !approvers.includes(accountable)) {
      approvers.push(`全体承認: ${accountable}`);
    }
  }
...
// 成果物作成・承認タイムラインを抽出する関数
function extractDeliverableTimeline(deliverables, timeline) {
  if (!timeline) return '';
  ...
  if (Array.isArray(timeline)) {
    timeline.forEach(item => {
      if (item && (item.includes('作成') || item.includes('承認') || item.includes('提出'))) {
        timelines.push(item);
      }
    });
  ...
  return timelines.length > 0 ? timelines.join('\n') : '別途スケジュール参照';
}

// ワークスペックからプロセスオーナーを抽出
function extractProcessOwnerFromWorkSpec(raci) {
  if (!raci) return '';

  if (typeof raci === 'object') {
    return raci.accountable || raci.A || '未設定';
  } else if (typeof raci === 'string') {
    return extractProcessOwner(raci);
  }

  return '未設定';
}

// ワークスペックからCEAVOP対応を抽出
function extractCEAVOPFromWorkSpec(workSpec) {
  const ceavopList = [];

  // タイトルと概要から判定
  const content = `${workSpec.title || ''} ${workSpec.summary || ''}`;

  if (content.includes('決算') || content.includes('財務')) {
    ceavopList.push('C:完全性', 'A:正確性', 'P:表示');
  }

  if (content.includes('監査') || content.includes('内部統制')) {
    ceavopList.push('E:存在性', 'O:発生', 'A:正確性');
  }

  if (content.includes('在庫') || content.includes('資産')) {
    ceavopList.push('E:存在性', 'V:評価');
  }

  if (content.includes('売上') || content.includes('収益')) {
    ceavopList.push('O:発生', 'C:完全性');
  }

  // security_privacy_controlsから判定
  if (workSpec.security_privacy_controls && Array.isArray(workSpec.security_privacy_controls)) {
    workSpec.security_privacy_controls.forEach(control => {
      if (control && control.includes('アクセス')) {
        ceavopList.push('O:発生（権限管理）');
      }
    });
  }

  return ceavopList.length > 0 ? [...new Set(ceavopList)].join('、') : '個別評価要';
}

// プロセスオーナーを抽出する関数
function extractProcessOwner(raciString) {
  if (!raciString) return '';

  // RACI形式から'A'（Accountable）を抽出
  const raciPattern = /A[：:]\s*([^、,]+)/;
  const match = raciString.match(raciPattern);
  if (match) {
    return match[1].trim();
  }

  // 「責任者」「管理者」「承認者」などのキーワードを探す
  const ownerPattern = /(責任者|管理者|承認者|決裁者)[：:]\s*([^、,]+)/;
  const ownerMatch = raciString.match(ownerPattern);
  if (ownerMatch) {
    return ownerMatch[2].trim();
  }

  // 最初の役職を返す（フォールバック）
  const firstRole = raciString.split(/[、,]/)[0];
  return firstRole ? firstRole.trim() : '';
}

// CEAVOP対応をチェックする関数
function checkCEAVOPControl(processName, workContent, condition, dept) {
  const ceavop = [];

  // C - Completeness（完全性）
  if (workContent.includes('漏れ') || workContent.includes('網羅') || workContent.includes('全件') ||
      workContent.includes('チェックリスト') || workContent.includes('確認')) {
    ceavop.push('C:完全性（全取引の記録）');
  }

  // E - Existence（存在性）
  if (workContent.includes('実在') || workContent.includes('確認') || workContent.includes('照合') ||
      workContent.includes('現物') || workContent.includes('実査')) {
    ceavop.push('E:存在性（実在性の確認）');
  }

  // A - Accuracy（正確性）
  if (workContent.includes('正確') || workContent.includes('計算') || workContent.includes('照合') ||
      workContent.includes('チェック') || workContent.includes('検証')) {
    ceavop.push('A:正確性（計算・記録の正確性）');
  }

  // V - Valuation（評価）
  if (workContent.includes('評価') || workContent.includes('査定') || workContent.includes('時価') ||
      workContent.includes('減損') || workContent.includes('引当')) {
    ceavop.push('V:評価（適切な評価基準）');
  }

  // O - Occurrence（発生）
  if (workContent.includes('承認') || workContent.includes('決裁') || workContent.includes('権限') ||
      workContent.includes('正当性')) {
    ceavop.push('O:発生（正当な取引）');
  }

  // P - Presentation（表示）
  if (workContent.includes('開示') || workContent.includes('報告') || workContent.includes('分類') ||
      workContent.includes('表示') || workContent.includes('注記')) {
    ceavop.push('P:表示（適切な開示・分類）');
  }

  // プロセス特有のCEAVOP判定
  if (processName.includes('決算')) {
    if (workContent.includes('締切') || workContent.includes('カットオフ')) {
      ceavop.push('C:期間帰属の完全性');
    }
    if (workContent.includes('仕訳')) {
      ceavop.push('A:仕訳の正確性');
    }
  }

  if (processName.includes('在庫') || processName.includes('棚卸')) {
    ceavop.push('E:在庫の実在性');
    ceavop.push('V:在庫評価の妥当性');
  }

  if (processName.includes('売上') || processName.includes('収益')) {
    ceavop.push('O:収益認識の正当性');
    ceavop.push('C:売上計上の完全性');
  }

  return ceavop.length > 0 ? ceavop.join('、') : 'なし';
}
