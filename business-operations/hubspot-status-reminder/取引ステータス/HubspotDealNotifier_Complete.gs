/**
 * HubSpot Deal Status Reminder - 完全版
...
function checkAndNotifyStaleDealsManual() {
  try {
    ...
    if (staleDeals.length > 0) {
      sendEmailNotification(staleDeals);
      ui.alert('通知完了', `${staleDeals.length}件の未更新案件を通知しました`, ui.ButtonSet.OK);
    } else {
...
<body>
      ...
      <table class="status-table">
        <thead>
          <tr>
            ...
            <th style="text-align: center;">平均停滞日数</th>
  ...
  Object.keys(statusSummary).sort((a, b) => statusSummary[b].count - statusSummary[a].count).forEach(status => {
    ...
    const amount = stat.totalAmount ? `¥${stat.totalAmount.toLocaleString()}` : '-';
    
    html += `
          <tr>
            <td>${status}</td>
            <td style="text-align: center;">${stat.count}</td>
            <td style="text-align: center;">${stat.avgDaysStale}日</td>
  ...
  if (urgentDealsList.length > 0) {
    urgentDealsList.forEach(deal => {
      const amount = deal.amount ? `¥${Number(deal.amount).toLocaleString()}` : '金額未設定';
      html += `
      <div class="deal-card">
        <div class="deal-title">
          <span class="badge-urgent">${deal.daysSinceUpdate}日停滞</span> 
          ${deal.dealName || '(名称未設定)'}
        </div>
        <div class="deal-info">
    ...
    if (staleDeals.filter(d => d.daysSinceUpdate >= 7).length > 5) {
      html += `<p style="font-size: 12px; color: #6c757d; margin-top: 10px;">他${staleDeals.filter(d => d.daysSinceUpdate >= 7).length - 5}件の緊急案件があります</p>`;
    }
  } else {
    html += `<p style="font-size: 13px; color: #6c757d;">現在、7日以上停滞している案件はありません</p>`;
  }
  ...
  staleDeals.forEach(deal => {
    ...
    html += `
        <tr class="${rowClass}">
          <td>${deal.dealName || '(名称未設定)'}</td>
          <td><span class="${daysBadgeClass}">${deal.daysSinceUpdate}日</span></td>
...
【ステータス別集計】\n`;
  ...
  Object.keys(statusSummary).sort((a, b) => statusSummary[b].count - statusSummary[a].count).forEach(status => {
    const stat = statusSummary[status];
    const amount = stat.totalAmount ? `¥${stat.totalAmount.toLocaleString()}` : '-';
    text += `・${status}: ${stat.count}件 (平均${stat.avgDaysStale}日停滞, 合計${amount})\n`;
  });
...
function testEmailNotification() {
  ...
  SpreadsheetApp.getUi().alert('テスト送信', 'テストメールを送信しました', SpreadsheetApp.getUi().ButtonSet.OK);
}
